...
class Exams(CSP):
    ...
    def var_constraints(self, A, a, B, b):
        ...
        if A[0] == B[0] and A[4] != "LAB" and B[4] != "LAB":
            # an to mikrotero mathima einai telefteo se mia mera tote ok
    ...
    def display(self, assignment):
        ...
        # How many days left
        # From total slots subtract n-1 weeks to find remaining slots
        # In remaining slots calculate how many days (3-slots) there are
        days = ceil((max_value - weeks * 21) / 3)

        # We counted from n-1 week
        if days == 7:  # This means we reached n week -> no remaining days
            weeks += 1
            days = 0
        ...
        # print(self.nassigns)
        print(weeks, "Εβδομάδες και", days, "ημέρες")
...
if __name__ == '__main__':
    ...
    # BT
    # schedule.display(backtracking_search(schedule))

    # schedule = Exams("Στοιχεία Μαθημάτων.csv")
    # FC
    # schedule.display(backtracking_search(schedule, mrv, lcv, forward_checking))

    # schedule = Exams("Στοιχεία Μαθημάτων.csv")
    # MAC
    # schedule.display(backtracking_search(schedule, mrv, lcv, mac))

    # schedule = Exams("Στοιχεία Μαθημάτων.csv")
    # min_conflicts
    # schedule.display(min_conflicts(schedule,)) """