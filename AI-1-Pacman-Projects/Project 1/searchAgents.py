...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # Start with a tuple of 4 corners. All not visited yet.
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # ((x,y),()) represents a state according to Hint 3
        # Return a tuple ((x,y),startState)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Goal is state[1] = (1, 1, 1, 1) if all corners are visited
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        # Make corners a list for customisation
        corners = list(state[1][:])
        explored = []
        # If corner is not visited
        if (nextx, nexty) in self.corners:
            if (nextx, nexty) not in explored:
                explored.append((nextx, nexty))
                # Corner is now visited
...
def cornersHeuristic(state, problem):
    ...
    for i, corner in enumerate(state[1]):
        ...
        # If not visited
        if corner == 0:
            # Use manhattan distance between current position and each corner
            # Then, include the worst case scenario and take max distance
            dist = max(dist, util.manhattanDistance(current, corners[i]))
    return dist
...
def foodHeuristic(state, problem):
    ...
    # For every food dot, find the "mazeDistance" from current position
    for dot in food:
        distances.append(mazeDistance(position, dot, startState))

    # Optimal solution is the longest of the distances
    opt = max(distances)
    return opt
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # Bfs searches shallowest leaves -> closest dots (Already written in search.py)
